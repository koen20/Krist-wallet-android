function wallet_refresh(dt)
	tim = tim + dt
	if tim > 100 then
		channel = love.thread.getChannel("action")
		channel:push("balance")
		channel:push("names")
		channel:push("transactions")	
		tim = 0
	end
	wallet_balance()
	wallet_transactions()
...
function wallet_transactions()
	...
	if value ~= nil then
		...
		repeat
			...
			thing = string.sub(body,13+(31*graphics),22+(31*graphics))
			table.insert(transactions, {date = date,thing = thing,amount = amount,y = yg,graphics = graphics})
			graphics = graphics + 1
			yg = yg + 30
		until graphics > 20
		value = nil
...
function wallet_domains()
	...
	if value ~= nil then
		...
		namelist = value
		repeat
			name = string.sub(namelist:sub(0,namelist:find(";")-1)..".kst",0,20)
			namelist = namelist:sub(1+namelist:find(";"))
			table.insert(names, {name = name,y = yn,tx = tx})
			yn = yn + 30
...
function wallet_top()
	...
	yt = 0
	amount = 0
	url = syncNode .. '?richapi'
	body,c,l,h = http.request(url)
	repeat
		peer = string.sub(body,1,10)
		body = string.sub(body,11)
		amount = string.sub(body,1,8)
		body = string.sub(body,9)
		date = string.sub(body,1,11)
		body = string.sub(body,12)
		table.insert(top, {date = date,peer = peer,amount = amount,y = yt,tx = tx})
		tx = tx + 1
		yt = yt + 30
    until tx > 16
end
...
function transfer(recipient,amount)
	url = syncNode .. '?pushtx2&q='..recipient..'&pkey='..masterkey..'&amt='..amount
	body,c,l,h = http.request(url)
end
